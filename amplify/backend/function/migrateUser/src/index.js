...
function createUser(username, password, user, callback) {
	...
	if (user.postalcode) {
		params.UserAttributes.push({ Name: 'postalcode', Value: user.postalcode })
	}

	if (user.department) {
		params.UserAttributes.push({ Name: 'department', Value: user.department })
	}

	if (user.mobile) {
		params.UserAttributes.push({ Name: 'phone number', Value: user.mobile })
	}
